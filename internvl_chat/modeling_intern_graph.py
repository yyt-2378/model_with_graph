...
from project.equiformer.nets import model_entrypoint
from project.equiformer.nets.instance_norm import EquivariantInstanceNorm
from project.equiformer.nets.graph_norm import EquivariantGraphNorm
from project.equiformer.nets.layer_norm import EquivariantLayerNormV2
from project.equiformer.nets.radial_func import RadialProfile
from project.equiformer.nets.tensor_product_rescale import (TensorProductRescale, LinearRS,
    FullyConnectedTensorProductRescale, irreps2gate)
from project.equiformer.nets.fast_activation import Activation, Gate
from project.equiformer.nets.drop import EquivariantDropout, EquivariantScalarsDropout, DropPath
from project.equiformer.nets.graph_attention_transformer import (get_norm_layer, 
...
from project.equiformer.nets.gaussian_rbf import GaussianRadialBasisLayer

from project.equiformer.ocp.ocpmodels.common.registry import registry
from project.equiformer.ocp.ocpmodels.common.utils import (
    conditional_grad,
    get_pbc_distances,
    radius_graph_pbc,
)
...
def model_factory(args):
    model = GraphAttentionTransformerOC20(irreps_node_embedding=args.irreps_node_embedding, 
                                          num_layers=args.num_layers,
                                          irreps_node_attr=args.irreps_node_attr,
                                          use_node_attr=args.use_node_attr,
                                          irreps_sh=args.irreps_sh,
                                          max_radius=args.max_radius,
                                          number_of_basis=args.number_of_basis, fc_neurons=args.fc_neurons, 
                                          use_atom_edge_attr=args.use_atom_edge_attr, irreps_atom_edge_attr=args.irreps_atom_edge_attr,
                                          irreps_feature=args.irreps_feature,
                                          irreps_head=args.irreps_head, num_heads=args.num_heads, irreps_pre_attn=args.irreps_pre_attn,
                                          rescale_degree=args.rescale_degree, nonlinear_message=args.nonlinear_message,
                                          irreps_mlp_mid=args.irreps_mlp_mid,
                                          norm_layer=args.norm_layer,
                                          alpha_drop=args.alpha_drop, proj_drop=args.proj_drop, out_drop=args.out_drop, drop_path_rate=args.drop_path_rate,
                                          otf_graph=args.otf_graph, use_pbc=args.use_pbc, max_neighbors=args.max_neighbors)

    if args.checkpoint_path is not None:
        state_dict = torch.load(args.checkpoint_path, map_location='cpu')
        new_state_dict = {}
        for key in state_dict['state_dict'].keys():
            # 删除前缀 'module.module.' 如果存在
            new_key = key.replace('module.module.', '')
...
def parse_args():
    parser = argparse.ArgumentParser(description='Graph Attention Transformer OC20 Model Parameters')
    default_device = 'cuda' if torch.cuda.is_available() else 'cpu'
    
    # Model specific parameters
    parser.add_argument('--irreps_node_embedding', type=str, default='256x0e+128x1e', help='Node embedding irreducible representations')
    parser.add_argument('--num_layers', type=int, default=6, help='Number of layers in the network')
    parser.add_argument('--irreps_node_attr', type=str, default='1x0e', help='Node attribute irreducible representations')
    parser.add_argument('--use_node_attr', action='store_true', default=False, help='Whether to use node attributes')
    parser.add_argument('--irreps_sh', type=str, default='1x0e+1x1e', help='Spherical harmonics irreducible representations')
    parser.add_argument('--max_radius', type=float, default=5.0, help='Maximum radius for graph construction')
    parser.add_argument('--number_of_basis', type=int, default=128, help='Number of basis functions')
    parser.add_argument('--fc_neurons', nargs='+', type=int, default=[64, 64], help='Fully connected layer neurons')
    parser.add_argument('--use_atom_edge_attr', action='store_true', default=False, help='Whether to use atom edge attributes')
    parser.add_argument('--irreps_atom_edge_attr', type=str, default='1x0e', help='Atom edge attribute irreducible representations')
    parser.add_argument('--irreps_feature', type=str, default='512x0e', help='Feature irreducible representations')
    parser.add_argument('--irreps_head', type=str, default='32x0e+16x1e', help='Head irreducible representations')
    parser.add_argument('--num_heads', type=int, default=8, help='Number of attention heads')
    parser.add_argument('--irreps_pre_attn', type=str, default='256x0e+128x1e', help='Pre-attention irreducible representations')
    parser.add_argument('--rescale_degree', action='store_true', default=False, help='Whether to rescale degree')
    parser.add_argument('--nonlinear_message', action='store_true', default=True, help='Whether to use nonlinear messages')
    parser.add_argument('--irreps_mlp_mid', type=str, default='768x0e+384x1e', help='Middle MLP irreducible representations')
    parser.add_argument('--norm_layer', type=str, default='layer', help='Type of normalization layer')
    parser.add_argument('--alpha_drop', type=float, default=0.2, help='Alpha dropout rate')
    parser.add_argument('--proj_drop', type=float, default=0.0, help='Projection dropout rate')
    parser.add_argument('--out_drop', type=float, default=0.0, help='Output dropout rate')
    parser.add_argument('--drop_path_rate', type=float, default=0.0, help='Drop path rate')
    parser.add_argument('--otf_graph', action='store_true', default=True, help='Whether to build graph on the fly')
    parser.add_argument('--use_pbc', action='store_true', default=True, help='Whether to use periodic boundary conditions')
    parser.add_argument('--max_neighbors', type=int, default=500, help='Maximum number of neighbors')
    parser.add_argument('--checkpoint_path', type=str, default='/mnt/petrelfs/yangyaotian/project/equiformer/oc20_weights/checkpoints/2022-04-18-03-14-08/best_checkpoint.pt', help='model weights')
